...
/**
 *  \file       conditionreportroom_document.php
 *  \ingroup    conditionreport
 *  \brief      Tab for documents linked to Conditionreportroom
 */
...
$backtopage = GETPOST('backtopage', 'alpha', 2);
...
if ($id > 0 || !empty($ref)) {
    $upload_dir = $conf->conditionreport->multidir_output[$object->entity ? $object->entity : $conf->entity] . "/conditionreportroom/" . get_exdir(0, 0, 0, 1, $object);
}
...
if ($enablepermissioncheck) {
    ...
    $permissiontoadd  = $user->hasRight('conditionreport', 'conditionreportroom', 'write'); // Used by the include of actions_addupdatedelete.inc.php and actions_linkedfiles.inc.php
} else {
    $permissiontoread = 1;
    $permissiontoadd  = 1;
}
...
if (strlen($backtopage) > 0) {
    header('Location: ' . $backtopage); // Pour reaffichage de la fiche en cours d'edition
}
/*
 * View
 */
...
// Header
// ------
$title    = $langs->trans("Conditionreportroom") . " - " . $langs->trans("Files");
//$title = $object->ref." - ".$langs->trans("Files");
$help_url = '';
//Example $help_url='EN:Module_Third_Parties|FR:Module_Tiers|ES:Empresas';
llxHeader('', $title, $help_url, '', 0, 0, '', '', '', 'mod-conditionreport page-card_document');

// Show tabs
$head = conditionreportroomPrepareHead($object);

print dol_get_fiche_head($head, 'document', $langs->trans("Conditionreportroom"), -1, $object->picto);
...
$modulepart             = 'conditionreport';
$param                  = '&id=' . $object->id;
//$relativepathwithnofile='conditionreportroom/' . dol_sanitizeFileName($object->id).'/';
