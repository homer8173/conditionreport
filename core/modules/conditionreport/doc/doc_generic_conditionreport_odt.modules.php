...
/**
 *	\file       htdocs/core/modules/conditionreport/doc/doc_generic_conditionreport_odt.modules.php
 *	\ingroup    conditionreport
 *	\brief      File of class to build ODT documents for conditionreports
 */
 ...
 */
class doc_generic_conditionreport_odt extends ModelePDFConditionreport
{
	/**
	 * Issuer
	 * @var Societe
	...
	public function info($langs)
	{
		...
		if ($nbofiles) {
			...
			foreach ($listoffiles as $file) {
				$texte .= '- '.$file['name'].' <a href="'.DOL_URL_ROOT.'/document.php?modulepart=doctemplates&file=conditionreport_conditionreport/'.urlencode(basename($file['name'])).'">'.img_picto('', 'listlight').'</a>';
	 ...
	 *
	 *	@param		Conditionreport	$object				Object source to build document
	...
	public function write_file($object, $outputlangs, $srctemplatepath, $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($conf->conditionreport->dir_output) {
			// If $object is id instead of object
			if (!is_object($object)) {
				$id = $object;
				$object = new Conditionreport($this->db);
			...
			$dir = $conf->conditionreport->multidir_output[isset($object->entity) ? $object->entity : 1];
			...
			if (file_exists($dir)) {
				...
				dol_mkdir($conf->conditionreport->dir_temp);
				if (!is_writable($conf->conditionreport->dir_temp)) {
					$this->error = $langs->transnoentities("ErrorFailedToWriteInTempDirectory", $conf->conditionreport->dir_temp);
				...
				try {
					$odfHandler = new Odf(
						$srctemplatepath,
						array(
						'PATH_TO_TMP'	  => $conf->conditionreport->dir_temp,
