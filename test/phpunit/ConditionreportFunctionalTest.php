...
/**
 * \file    test/phpunit/ConditionreportFunctionalTest.php
...
/**
 * Class ConditionreportFunctionalTest
 *
 ...
 *
 * @package Testconditionreport
 */
class ConditionreportFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	private static $module_id = 500045; // TODO: autodetect?
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modConditionreport')->click();
		} else {
			// Disable the module
			$this->byHref('modConditionreport')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/conditionreport/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('conditionreport/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/conditionreport/admin/about.php');
		$this->authenticate();
		return $this->assertContains('conditionreport/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('conditionreportwidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modConditionreport_ConditionreportTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modConditionreport_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
