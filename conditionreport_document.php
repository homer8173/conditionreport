...
/**
 *  \file       conditionreport_document.php
 *  \ingroup    conditionreport
 *  \brief      Tab for documents linked to Conditionreport
 */
...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->conditionreport->multidir_output[$object->entity ? $object->entity : $conf->entity]."/conditionreport/".get_exdir(0, 0, 0, 1, $object);
}
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd  = $user->hasRight('conditionreport', 'conditionreport', 'write'); // Used by the include of actions_addupdatedelete.inc.php and actions_linkedfiles.inc.php
} else {
	$permissiontoread = 1;
	$permissiontoadd  = 1;
}
...
// Header
// ------
$title = $langs->trans("Conditionreport")." - ".$langs->trans("Files");
...
print dol_get_fiche_head($head, 'document', $langs->trans("Conditionreport"), -1, $object->picto);
...
$param = '&id='.$object->id;
//$relativepathwithnofile='conditionreport/' . dol_sanitizeFileName($object->id).'/';
$relativepathwithnofile = 'conditionreport/'.dol_sanitizeFileName($object->ref).'/';
