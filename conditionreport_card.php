...
/**
 *    \file       conditionreport_card.php
 *    \ingroup    conditionreport
 *    \brief      Page to create/edit/view conditionreport
 */
...
if ($enablepermissioncheck) {
    ...
    $permissiontodelete = $user->hasRight('conditionreport', 'conditionreport', 'delete') || ($permissiontoadd && isset($object->status) && $object->status == $object::STATUS_DRAFT);
    $permissionnote     = $user->hasRight('conditionreport', 'conditionreport', 'write'); // Used by the include of actions_setnotes.inc.php
    $permissiondellink  = $user->hasRight('conditionreport', 'conditionreport', 'write'); // Used by the include of actions_dellink.inc.php
} else {
    $permissiontoread   = 1;
...
$upload_dir = $conf->conditionreport->multidir_output[isset($object->entity) ? $object->entity : 1] . '/conditionreport';
...
if (empty($reshook)) {
    ...
    $backurlforlist = dol_buildpath('/conditionreport/conditionreport_list.php', 1);
    ...
    if (empty($backtopage) || ($cancel && empty($id))) {
        if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
            if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
                ...
                $backtopage = dol_buildpath('/conditionreport/conditionreport_card.php', 1) . '?id=' . ((!empty($id) && $id > 0) ? $id : '__ID__');
            }
        }
    }

    $triggermodname = 'CONDITIONREPORT_CONDITIONREPORT_MODIFY'; // Name of trigger action code to execute when we modify record
    ...
    $triggersendname = 'CONDITIONREPORT_SENTBYMAIL';
    $autocopy        = 'MAIN_MAIL_AUTOCOPY_CONDITIONREPORT_TO';
...
$title = $langs->trans("Conditionreport") . " - " . $langs->trans('Card');
//$title = $object->ref." - ".$langs->trans('Card');
if ($action == 'create') {
    $title = $langs->trans("NewObject", $langs->transnoentitiesnoconv("Conditionreport"));
}
$help_url = '';
...
if (($id || $ref) && $action == 'edit') {
    print load_fiche_titre($langs->trans("Conditionreport"), '', 'object_' . $object->picto);
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
    ...
    print dol_get_fiche_head($head, 'card', $langs->trans("Conditionreport"), -1, $object->picto, 0, '', '', 0, '', 1);

    $formconfirm = '';

    // Confirmation to delete (using preloaded confirm popup)
    if ($action == 'delete' || ($conf->use_javascript_ajax && empty($conf->dol_use_jmobile))) {
        $formconfirm = $form->formconfirm($_SERVER["PHP_SELF"] . '?id=' . $object->id, $langs->trans('DeleteConditionreport'), $langs->trans('ConfirmDeleteObject'), 'confirm_delete', '', 0, 'action-delete');
    }
    ...
    if ($action == 'xxx') {
        $text = $langs->trans('ConfirmActionConditionreport', $object->ref);
    ...
    if ($object->status == $object::STATUS_DRAFT) {
        print '<td colspan="2">';
        print $langs->trans("createRoomModele") . " :<br />";
    ...
    if ($action != 'presend') {
        ...
        if ($includedocgeneration) {
            ...
            print $formfile->showdocuments('conditionreport:Conditionreport', $object->element . '/' . $objref, $filedir, $urlsource, $genallowed, $delallowed, $object->model_pdf, 1, 0, 0, 28, 0, '', '', '', $langs->defaultlang);
        }
        ...
        $morehtmlcenter = dolGetButtonTitle($langs->trans('SeeAll'), '', 'fa fa-bars imgforviewmode', dol_buildpath('/conditionreport/conditionreport_agenda.php', 1) . '?id=' . $object->id);
