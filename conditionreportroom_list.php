...
/**
 *   	\file       conditionreportroom_list.php
 *		\ingroup    conditionreport
 *		\brief      List page for conditionreportroom
 */
...
require_once __DIR__.'/class/conditionreportroom.class.php';
...
if ($enablepermissioncheck) {
	...
	$permissiontodelete = $user->hasRight('conditionreport', 'conditionreportroom', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("conditionreport")) {
	accessforbidden('Module conditionreport not enabled');
}
if (!$permissiontoread) {
	accessforbidden();
}
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'Conditionreportroom';
	$objectlabel = 'Conditionreportroom';
	$uploaddir = $conf->conditionreport->dir_output;
...
$title = $langs->trans("Conditionreportrooms");
//$help_url = "EN:Module_Conditionreportroom|FR:Module_Conditionreportroom_FR|ES:Módulo_Conditionreportroom";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/conditionreport/conditionreportroom_card.php', 1).'?id='.((int) $id));
	exit;
}
...
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'mod-conditionreport page-list bodyforlist');	// Can use also classforhorizontalscrolloftabs instead of bodyforlist for no horizontal scroll
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/conditionreport/conditionreportroom_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendConditionreportroomRef";
$modelmail = "conditionreportroom";
$objecttmp = new Conditionreportroom($db);
